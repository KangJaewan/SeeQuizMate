    ...
    def grade_single_answer(answer: QuizAnswer) -> QuizResult:
        ...
        try:
            ...
            if answer.quiz_type == "multiple_choice":
                # 객관식: 선택지 인덱스 비교 (case-insensitive, whitespace-trimmed)
                is_correct = (
                    str(answer.user_answer).strip().lower()
                    == str(answer.correct_answer).strip().lower()
                )
                ...
                # 단답형: 문자열 비교 (case-insensitive, whitespace-trimmed, strict equality)
                is_correct = (
                    str(answer.user_answer).strip().lower()
                    == str(answer.correct_answer).strip().lower()
                )
