...
async def list_folders(limit: int = 100, skip: int = 0):
    ...
    try:
        ...
        for folder in sorted(
            folders,
            key=lambda x: x.get('last_accessed_at') or x.get('created_at') or datetime.min,
            reverse=True
        ):
            ...
            folder_responses.append(FolderResponse(
                ...
                created_at=folder.get("created_at", datetime.min),
                last_accessed_at=folder.get("last_accessed_at", datetime.min),
